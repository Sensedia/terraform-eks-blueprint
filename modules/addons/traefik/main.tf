...
resource "kubectl_manifest" "alb_traefik_ingress" {
  count = var.install && var.create_ingress && var.ingress_certificate_arn != null && length(var.ingress_certificate_arn) != 0 ? 1 : 0
  ...
  metadata:
    ...
    annotations:
      ...
      alb.ingress.kubernetes.io/inbound-cidrs: "${var.ingress_inbound_cidrs}"
